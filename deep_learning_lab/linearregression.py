...
kc_x_train, kc_x_valid, kc_y_train, kc_y_valid = train_test_split(kc_data.iloc[:,0:6], kc_data.iloc[:,6],
                                                    test_size=0.3, random_state=87)
np.random.seed(155)
def norm_statistics(df1, df2):
    dfs = df1.append(df2)
    minimum = np.min(dfs)
    maximum = np.max(dfs)
    mu = np.mean(dfs)
...
def z_score(col, stats):
    ...
    return df2
stats = norm_statistics(kc_x_train, kc_x_valid)
...
def basicmodel2(x_size, y_size):
    ...
    return(t_model)
model = basicmodel2(arr_x_train.shape[1], 1)
